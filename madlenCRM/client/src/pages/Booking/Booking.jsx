    ...
    function generateSlots(start, end, durationMinutes = 60) {
        ...
        // Знайти рядок з comment і додати під ним:
        let cur = sh * 60 + sm;
        let endMin = eh * 60 + em;
    ...
    export default function Booking() {
        ...
        const [clientWishes, setClientWishes] = useState(''); // Нове поле для кольору
        ...
        const handleSubmit = async () => {
            ...
            if (!user) {
                setError('Потрібно авторизуватися, щоб зробити запис');
                setSubmitting(false);
                return;
            }
            ...
            try {
                await api.post('/appointments', {
                    ...
                    time:    selectedTime,
                    comment,
                    // 1. ФІКС: Додаємо збереження побажань
                    clientWishes: clientWishes,
                    client: user.id || user._id,
                    clientName: user.name,
                    // 2. ФІКС: Передаємо реальний телефон юзера замість нулів
                ...
                <div className="booking-body">
                    ...
                    {step === 4 && (
                        <div className="booking-step">
                            ...
                            {/* 🎨 БЛОК ДЛЯ ФАРБУВАННЯ (показуємо першим) */}
                            {/фарб|color|dye/i.test(selectedService?.name || "") && (
                                <div className="wishes-input-group" style={{ marginTop: '20px', textAlign: 'left' }}>
                                    <label style={{ color: '#D4AF37', fontSize: '11px', fontWeight: '900', textTransform: 'uppercase', marginBottom: '8px', display: 'block', letterSpacing: '1px' }}>
                                        ✨ Який колір або результат ви бажаєте?
                                    </label>
                                    <textarea
                                        className="comment-input"
                                        placeholder="Наприклад: хочу холодний блонд, або як на фото в профілі..."
                                        value={clientWishes}
                                        onChange={e => setClientWishes(e.target.value)}
                                        style={{ minHeight: '80px', borderColor: 'rgba(212, 175, 55, 0.3)', marginBottom: '15px' }}
                                    />
                                </div>
                            )}

                            {/* 📝 ЗАГАЛЬНИЙ КОМЕНТАР (під побажанням) */}
                            <textarea
                                className="comment-input"
                                placeholder="Додаткові замітки для майстра..."
                                value={comment}
